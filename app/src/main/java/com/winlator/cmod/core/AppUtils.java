...
public abstract class AppUtils {
    ...
    public static boolean setSpinnerSelectionFromValue(Spinner spinner, String value) {
        return setSpinnerSelectionFromValue(spinner, value, false);
    }

    /**
     * Selects the spinner item matching {@code value}. If {@code addIfMissing} is true and no
     * match is found, the saved value is inserted into the spinner's own adapter and selected
     * instead of silently falling back to position 0. This matters for content-backed spinners
     * (wine/box64/FEXCore versions, presets, drivers): the underlying container config always
     * keeps the user's real saved choice, but the spinner list is rebuilt from whatever content
     * happens to be installed/synced at the moment the screen opens, and can momentarily be out
     * of sync with it (e.g. right after a fresh install, or if content was moved/renamed). Falling
     * back to index 0 in that case both looks wrong AND silently overwrites the user's real
     * choice with the wrong value the next time they hit Save without noticing.
     */
    public static boolean setSpinnerSelectionFromValue(Spinner spinner, String value, boolean addIfMissing) {
        ...
        if (addIfMissing && value != null && !value.isEmpty() && insertMissingSpinnerValue(spinner, value)) {
            return true;
        }
    ...
    private static boolean insertMissingSpinnerValue(Spinner spinner, String value) {
        android.widget.SpinnerAdapter adapter = spinner.getAdapter();
        if (!(adapter instanceof android.widget.ArrayAdapter)) return false;
        android.widget.ArrayAdapter<String> arrayAdapter = (android.widget.ArrayAdapter<String>) adapter;
