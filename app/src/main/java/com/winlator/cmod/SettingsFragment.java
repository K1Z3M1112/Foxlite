...
//        applyFieldSetLabelStyle(shortcutSettingsLabel, isDarkMode);
    ...
    private void setupLosslessDllImport(View view) {
        ...
        View logButton = view.findViewById(R.id.BTViewLsfgLog);
        if (logButton != null) {
            logButton.setOnClickListener(v -> {
                if (getContext() != null) {
                    startActivity(new android.content.Intent(getContext(), LogViewerActivity.class));
                }
            });
        }

        CheckBox logCaptureCheckBox = view.findViewById(R.id.CBEnableLogCapture);
        if (logCaptureCheckBox != null && getContext() != null) {
            logCaptureCheckBox.setChecked(AppLogCollector.isEnabled(getContext()));
            logCaptureCheckBox.setOnCheckedChangeListener((buttonView, isChecked) -> {
                if (getContext() != null) {
                    AppLogCollector.getInstance().setEnabled(getContext(), isChecked);
                }
            });
        }
    }
