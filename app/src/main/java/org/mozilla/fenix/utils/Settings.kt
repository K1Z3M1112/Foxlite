...
class Settings(private val appContext: Context) : PreferencesHolder {
    ...
    /**
     * "RAM Saver Mode": when enabled, only the currently selected tab is allowed to keep
     * a live engine session (GeckoView content process). Every other tab has its engine
     * session unlinked as soon as it stops being the active tab, so it stops consuming
     * RAM/CPU immediately instead of waiting for a low-memory signal from the OS. Tab
     * state (URL, scroll position, etc.) is preserved and the page is transparently
     * reloaded the next time the user switches back to that tab.
     */
    var ramSaverModeEnabled by booleanPreference(
        appContext.getPreferenceKey(R.string.pref_key_ram_saver_mode),
        default = false,
    )
